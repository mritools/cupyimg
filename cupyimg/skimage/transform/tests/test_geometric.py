...
from cupyimg.skimage.transform._geometric import GeometricTransform
...
@pytest.mark.parametrize("xp", [np, cp])  # , [np, cp]
...
def test_fundamental_matrix_residuals(xp):
    essential_matrix_tform = EssentialMatrixTransform(
        rotation=xp.eye(3), translation=xp.asarray([1, 0, 0]), xp=xp
    )
    tform = FundamentalMatrixTransform()
    tform.params = essential_matrix_tform.params
    src = xp.asarray([[0, 0], [0, 0], [0, 0]])
    dst = xp.asarray([[2, 0], [2, 1], [2, 2]])
    assert_array_almost_equal(
        tform.residuals(src, dst) ** 2, xp.asarray([0, 0.5, 2])
    )
...
def test_fundamental_matrix_forward(xp):
    ...
    tform = FundamentalMatrixTransform()
    tform.params = essential_matrix_tform.params
    src = xp.asarray([[0, 0], [0, 1], [1, 1]])
    assert_array_almost_equal(
        tform(src), xp.asarray([[0, -1, 0], [0, -1, 1], [0, -1, 1]])
    )
...
def test_fundamental_matrix_inverse(xp):
    ...
    src = xp.asarray([[0, 0], [0, 1], [1, 1]])
    assert_array_almost_equal(
        tform.inverse(src), xp.asarray([[0, 1, 0], [0, 1, -1], [0, 1, -1]])
    )
...
def test_essential_matrix_init(xp):
    ...
    assert_array_equal(
        tform.params, xp.asarray([0, -1, 0, 1, 0, 0, 0, 0, 0]).reshape(3, 3)
    )
