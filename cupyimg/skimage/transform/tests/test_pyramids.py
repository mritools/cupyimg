...
from cupyimg.skimage.transform import pyramids
...
def test_pyramid_reduce_gray():
    ...
    out1 = pyramids.pyramid_reduce(image_gray, downscale=2, multichannel=False)
    assert_array_equal(out1.shape, (rows / 2, cols / 2))
    assert_almost_equal(out1.get().ptp(), 1.0, decimal=2)
    out2 = pyramids.pyramid_reduce(
        image_gray, downscale=2, multichannel=False, preserve_range=True
    )
    assert_almost_equal(
        out2.get().ptp() / image_gray.get().ptp(), 1.0, decimal=2
    )
...
def test_pyramid_expand_gray():
    rows, cols = image_gray.shape
    out = pyramids.pyramid_expand(image_gray, upscale=2, multichannel=False)
...
def test_pyramid_expand_nd():
    for ndim in [1, 2, 3, 4]:
        ...
        out = pyramids.pyramid_expand(img, upscale=2, multichannel=False)
        expected_shape = cp.asarray(img.shape) * 2
        assert_array_equal(out.shape, expected_shape)
...
def test_build_gaussian_pyramid_rgb():
    rows, cols, dim = image.shape
    pyramid = pyramids.pyramid_gaussian(image, downscale=2, multichannel=True)
...
def test_build_gaussian_pyramid_gray():
    ...
    pyramid = pyramids.pyramid_gaussian(
        image_gray, downscale=2, multichannel=False
    )
    for layer, out in enumerate(pyramid):
...
def test_build_gaussian_pyramid_nd():
    for ndim in [1, 2, 3, 4]:
        ...
        pyramid = pyramids.pyramid_gaussian(
            img, downscale=2, multichannel=False
        )
        for layer, out in enumerate(pyramid):
...
def test_build_laplacian_pyramid_rgb():
    rows, cols, dim = image.shape
    pyramid = pyramids.pyramid_laplacian(image, downscale=2, multichannel=True)
    for layer, out in enumerate(pyramid):
        layer_shape = (rows / 2 ** layer, cols / 2 ** layer, dim)
        assert_array_equal(out.shape, layer_shape)
...
def test_laplacian_pyramid_max_layers():
    for downscale in [2, 3, 5, 7]:
        ...
        pyramid = pyramids.pyramid_laplacian(
            img, downscale=downscale, multichannel=False
        )
