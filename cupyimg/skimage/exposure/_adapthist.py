...
def _clahe(image, kernel_size, clip_limit, nbins):
    ...
    """
    row_step = int(image.shape[0] // math.ceil(image.shape[0] / kernel_size[0]))
    col_step = int(image.shape[1] // math.ceil(image.shape[1] / kernel_size[1]))

    nr = image.shape[0] // row_step
    nc = image.shape[1] // col_step

    bin_size = 1 + NR_OF_GRAY // nbins
    lut = cupy.arange(NR_OF_GRAY)
    lut //= bin_size

    map_array = cupy.zeros((nr, nc, nbins), dtype=int)
    ...
    for r in range(nr):
        ...
        for c in range(nc):
            ...
            hist = cupy.bincount(lut[sub_img.ravel()], minlength=nbins)
    ...
    for r in range(nr + 1):
        ...
        # Note: for CuPy, refactored to use slice object instead of cupy.arange
...
def clip_histogram(hist, clip_limit):
    ...
    mid_mask = cupy.logical_and(hist >= upper, hist < clip_limit)
    mid = hist[mid_mask]
    ...
    while n_excess > 0:  # Redistribute remaining excess
        ...
        for index in range(hist.size):
            ...
            step_size = max(1, cupy.count_nonzero(under_mask) // n_excess)
            under_mask = under_mask[index::step_size]
            hist[index::step_size][under_mask] += 1
            n_excess -= cupy.count_nonzero(under_mask)
            if n_excess <= 0:
                break
...
def map_histogram(hist, min_val, max_val, n_pixels):
    ...
    """
    out = cupy.cumsum(hist).astype(float)
    out *= (max_val - min_val) / n_pixels
    out += min_val
    cupy.clip(out, a_min=None, a_max=max_val, out=out)
...
def interpolate(image, xslice, yslice, mapLU, mapRU, mapLB, mapRB, lut):
    ...
    x_coef, y_coef = cupy.meshgrid(cupy.arange(x_size), cupy.arange(y_size))
    x_inv_coef, y_inv_coef = x_coef[:, ::-1] + 1, y_coef[::-1] + 1

    im_slice = lut[view]
    new = y_inv_coef * (
        x_inv_coef * mapLU[im_slice] + x_coef * mapRU[im_slice]
