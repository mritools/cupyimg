...
def test_peak_uint_range_dtype():
    im = cp.asarray([10, 100], dtype=np.uint8)
...
@pytest.mark.skip("cupy.histogram doesn't support the range argument")
...
def test_intensity_range_float(test_input, expected):
    image = cp.asarray([0.1, 0.2], dtype=np.float64)
    out = intensity_range(image, range_values=test_input)
    assert_array_equal(out, expected)
...
def test_adapthist_grayscale():
    ...
    assert_almost_equal(peak_snr(img, adapted).get(), 102.078, 3)
