...
def phase_cross_correlation(
    ...
    shifts : ndarray
        ...
        ``reference_image``.  Axis ordering is consistent with numpy
        (e.g. Z, Y, X)
    error : float
        Translation invariant normalized RMS error between ``reference_image``
        and ``moving_image``.
    phasediff : float
    ...
    shifts = cp.stack([m.astype(np.float64, copy=False) for m in maxima])
    ...
    # If upsampling > 1, then refine estimate with matrix multiply DFT
        ...
        maxima = (
            cp.stack([m.astype(np.float64, copy=False) for m in maxima])
            - dftshift
        )
