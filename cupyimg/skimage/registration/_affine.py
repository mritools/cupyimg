...
def _parameter_vector_to_matrix(parameter_vector):
    ...
    """
    parameter_vector = cp.asarray(parameter_vector)
    m = parameter_vector.shape[0]
    ndim = int((sqrt(4 * m + 1) - 1) / 2)
    top_matrix = cp.reshape(parameter_vector, (ndim, ndim + 1))
    bottom_row = cp.array([[0] * ndim + [1]])
    return cp.concatenate((top_matrix, bottom_row), axis=0)
...
def affine(reference_image, moving_image, *,
    ...
    **kwargs : keyword arguments
    ...
    >>> reference_image = cp.asarray(astronaut()[..., 1])
    >>> r = -0.12  # radians
    >>> c, s = np.cos(r), np.sin(r)
    >>> matrix_transform = cp.asarray([[c, -s, 0], [s, c, 50], [0, 0, 1]])
