...
# Note: in SciPy _ndim_coords_from_arrays is in Cython file interpnd.pyx.
...
class RegularGridInterpolator(object):
    ...
    *nearest*) may be chosen at each evaluation.
    ...
    >>> from cupyimg.scipy.interpolate import RegularGridInterpolator
    >>> def f(x, y, z):
    ...     return 2 * x**3 + 3 * y**2 - z
    >>> x = cp.linspace(1, 4, 11)
    >>> y = cp.linspace(4, 7, 22)
    >>> z = cp.linspace(7, 9, 33)
    >>> data = f(*cp.meshgrid(x, y, z, indexing='ij', sparse=True))
...
def interpn(
    points, values, xi, method="linear", bounds_error=True, fill_value=cp.nan
):
    """
    ...
    if bounds_error:
        for i, p in enumerate(xi.T):
            if not cp.logical_and(
                cp.all(grid[i][0] <= p), cp.all(p <= grid[i][-1])
            ):
                raise ValueError(
                    "One of the requested xi is out of bounds "
                    "in dimension %d" % i
