...
class TestMapCoordinates(unittest.TestCase):
    ...
    @numpy_cupyimg_allclose(atol=1e-4, rtol=1e-4, scipy_name="scp")
    def test_map_coordinates_float(self, xp, scp, dtype):
        a = testing.shaped_random((100, 100), xp, dtype)
        if not scipy1_6 and self.order > 1 and self.mode != "mirror":
            # Note: SciPy < 1.6.0 only correct for boundary='mirror'
            return a
    ...
    def test_map_coordinates_int(self, xp, scp, dtype):
        ...
        float_out = (
            self._map_coordinates(xp, scp, a.astype(xp.float64), coordinates)
            % 1
        )
...
class TestAffineTransform(unittest.TestCase):
    ...
    def test_affine_transform_int(self, xp, scp, dtype):
        ...
        float_out = (
            self._affine_transform(xp, scp, a.astype(xp.float64), matrix) % 1
        )
        half = xp.full_like(float_out, 0.5)
        out[xp.isclose(float_out, half, atol=1e-5)] = 0
        return out
...
class TestAffineTransformOpenCV(unittest.TestCase):
    ...
    # arithmetic.
    @numpy_cupyimg_allclose(atol=0.2)
    def test_affine_transform_opencv(self, xp, dtype):
        a = testing.shaped_random((100, 100), xp, dtype)
        matrix = testing.shaped_random((2, 3), xp, dtype)
        if xp == cupy:
            return cupyimg.scipy.ndimage.affine_transform(
                a, matrix, order=1, mode="opencv"
            )
        else:
...
@testing.parameterize(
    {"axes": (-1, -2)}, {"axes": (0, 1)}, {"axes": (2, 0)}, {"axes": (-2, 2)},
)
@testing.gpu
@testing.with_requires("scipy")
...
class TestRotateOpenCV(unittest.TestCase):
    ...
    @testing.for_float_dtypes(no_float16=True)
    @numpy_cupyimg_allclose(atol=0.3)
    def test_rotate_opencv(self, xp, dtype):
        a = testing.shaped_random((100, 100), xp, dtype)
        if xp == cupy:
            return cupyimg.scipy.ndimage.rotate(
                a, 10, reshape=False, order=1, mode="opencv"
            )
        else:
...
class TestShiftOpenCV(unittest.TestCase):
    ...
    def test_shift_opencv(self, xp, dtype):
        ...
        if xp == cupy:
            return cupyimg.scipy.ndimage.shift(a, shift, order=1, mode="opencv")
        else:
...
class TestZoomOpenCV(unittest.TestCase):
    ...
    @testing.for_float_dtypes(no_float16=True)
    @numpy_cupyimg_allclose(atol=1e-4)
    def test_zoom_opencv(self, xp, dtype):
        a = testing.shaped_random((100, 100), xp, dtype)
        if xp == cupy:
            return cupyimg.scipy.ndimage.zoom(
                a, self.zoom, order=1, mode="opencv"
            )
        else:
