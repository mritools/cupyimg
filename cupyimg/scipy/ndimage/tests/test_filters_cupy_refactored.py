    ...
    *(
        testing.product(
            [
                ...
                testing.product({"filter": ["convolve", "correlate"]})
                + testing.product(
                    {
                        "filter": [
                            "convolve1d",
                ...
                + testing.product(
                    {**COMMON_PARAMS, "mode": ["nearest", "wrap"]}
                )
                + testing.product(
                    {
                        **COMMON_PARAMS,
...
class TestSpecialWeightCases(FilterTestCaseBase):
    @testing.numpy_cupy_allclose(
        atol=1e-5, rtol=1e-5, scipy_name="scp", accept_error=ValueError
    )
    ...
    @testing.numpy_cupy_raises(
        scipy_name="scp", accept_error=(RuntimeError, ValueError)
    )
